...
const MongleLetter = ({ diaries }) => {
    ...
    return (
        <div
            className="rounded-3xl shadow-sm p-6 border-2"
            style={{
                background: `linear-gradient(to bottom right, ${bgFrom}, ${bgFrom})`,
        ...
        >
            ...
            <div className="flex items-center justify-between mb-4">
                <div className="flex items-center gap-2">
                    <FaEnvelope className="text-2xl" style={{ color: accentColor }} />
                    <h3 className="text-lg font-cute text-gray-700">몽글이의 편지</h3>
                </div>
                <SmartEmoji value={message.emoji} size={30} />
            </div>

            {/* 편지 내용 */}
            <div
                className="bg-white/60 rounded-2xl p-5 border"
                style={{ borderColor: `${primaryColor}4D` }}
            >
                <h4 className="text-base font-cute mb-3" style={{ color: accentColor }}>
                    {message.title}
                </h4>
            ...
            <div className="mt-4 text-right">
                <p className="text-xs font-cute text-gray-400">
                    사랑을 담아, 몽글이 <MongleIcon name="heart" size={12} color="#FFB5C2" className="inline-block" />
                </p>
            </div>
        </div>
    );
