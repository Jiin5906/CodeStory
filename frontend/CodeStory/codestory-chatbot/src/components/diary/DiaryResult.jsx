...
const DiaryResult = ({ data, onHome }) => {
    ...
    return (
        <div className="flex flex-col h-full animate-fade-in-up max-w-2xl mx-auto w-full">
            ...
            <div className="bg-zinc-900 border border-zinc-800 rounded-3xl p-6 shadow-2xl relative overflow-hidden">
                ...
                <div className="flex justify-between items-start mb-6 border-b border-zinc-800 pb-4">
                    ...
                    <div className="text-right text-sm space-y-1">
                        <p><span className="text-zinc-500">Tension:</span> <span className={getMoodColor(data.tension)}>{data.tension}</span></p>
                        <p><span className="text-zinc-500">Mood:</span> <span className={getMoodColor(data.mood)}>{data.mood}</span></p>
                        <p><span className="text-zinc-500">Fun:</span> <span className={getMoodColor(data.fun)}>{data.fun}</span></p>
                    </div>
                </div>
                ...
                <div className="mb-8">
                    ...
                    <div className="mt-4 flex flex-wrap gap-2">
                        {data.tags?.map(tag => (
                            <span key={tag} className="text-xs text-pink-400">#{tag}</span>
                        ))}
                    </div>
                </div>

                {/* AI 포스트잇 (코멘트) */}
                <div className="bg-yellow-100/10 border border-yellow-200/20 p-4 rounded-xl relative transform rotate-1">
